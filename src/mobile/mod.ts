 ...
 *
 * **专用**：`MobilePortalHostScope` / `MobilePortalHostContext`（将 Portal 限制在预览视口内）；`ScrollList`；以及 `form` / `feedback`（**含 `Dialog`**，与 `desktop/feedback` 同实现 re-export，见本文件 `// mobile/feedback（增量）`；另有 BottomSheet、ActionSheet 等）/ `navigation` / `layout` / `charts` / `config-provider` / `data-display` 子路径的移动端入口。
...
/**
 * 显式再导出（替代 `export *`）：shared 并集 + 本目录 form/feedback/navigation + 专用组件。
 */
...
export {
  ...
  defaultAutoCompleteMessages,
  defaultColorPickerMessages,
  defaultDateTimePickerMessages,
  defaultFormListMessages,
  defaultGetUploadResultUrl,
  defaultMarkdownEditorMessages,
  defaultMentionsMessages,
  defaultPasswordMessages,
  defaultRateMessages,
  defaultRichTextEditorMessages,
  defaultSelectMessages,
  defaultSliderMessages,
  defaultTextareaMessages,
  defaultTransferMessages,
  defaultTreeSelectMessages,
...
// mobile/form（增量）（无相对 shared 的增量符号，省略）
