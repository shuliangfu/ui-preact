/**
 * @module @dreamer/ui-preact
 * @packageDocumentation
 *
 * @description
 * **Preact** UI 组件库（自 `ui-view` 同源拆分迁移）。`jsxImportSource` 为 `preact`。
 *
 * **已迁移**：`basic`；`form` 子集（含 Cascader、TreeSelect、Select、MultiSelect、**DatePicker**、PickerCalendarNav 等）；`layout`；`navigation`（Anchor、PageHeader、BackTop、Pagination、Steps、Menu、Sidebar、Affix）；`feedback`（portal、Toast/Message/Notification、Alert、Drawer、Progress、Result）；`data-display`（Calendar、Tag、Empty、Statistic、Segmented、Descriptions、Card、List、calendar-utils）；{@link ./mobile/MobilePortalHostScope.tsx}。
 * **仍待与 ui-view 对齐**：TimePicker、DateTimePicker、ColorPicker、Upload 链、Mentions、Transfer、RichText、Markdown、charts、data-display 其余（Collapse、Tree、Image、Comment 等）。
 *
 * ## 子路径（`deno.json` → `exports`）
 *
 * | 导入子路径 | 说明 |
 * |------------|------|
 * | `@dreamer/ui-preact` | 桌面聚合：shared + form |
 * | `@dreamer/ui-preact/basic` | 桌面 basic |
 * | `@dreamer/ui-preact/form` | 桌面表单 |
 * | `@dreamer/ui-preact/shared` | 类型 + basic + form + layout + navigation + feedback + data-display |
 * | `@dreamer/ui-preact/mobile` | shared + mobile/form + Portal Scope |
 * | `@dreamer/ui-preact/mobile/basic` | 移动 basic |
 * | `@dreamer/ui-preact/mobile/form` | 移动表单（同源 re-export） |
 * | `@dreamer/ui-preact/layout` | 布局组件 |
 * | `@dreamer/ui-preact/feedback` | 反馈与全局提示 |
 * | `@dreamer/ui-preact/navigation` | 导航组件 |
 * | `@dreamer/ui-preact/data-display` | Calendar 等 |
 */

/**
 * 桌面默认入口见 {@link ./desktop/mod.ts}；以下显式从该模块转发（避免 `export *`）。
 */
// desktop 聚合（根入口）
export {
  Accordion,
  Affix,
  Alert,
  Anchor,
  AutoComplete,
  Avatar,
  BackTop,
  Badge,
  Breadcrumb,
  Button,
  ButtonGroup,
  Calendar,
  Card,
  Carousel,
  Cascader,
  ChartBar,
  ChartBase,
  ChartBubble,
  ChartDoughnut,
  ChartLine,
  ChartPie,
  ChartPolarArea,
  ChartRadar,
  ChartScatter,
  Checkbox,
  CheckboxGroup,
  clearMessages,
  clearToasts,
  closeNotification,
  CodeBlock,
  Collapse,
  ColorPicker,
  Comment,
  commitMaybeSignal,
  compareCalendarDays,
  ConfigProvider,
  Container,
  DatePicker,
  DateTimePicker,
  DEFAULT_UPLOAD_CHUNK_SIZE,
  defaultAutoCompleteMessages,
  defaultCarouselMessages,
  defaultColorPickerMessages,
  defaultDateTimePickerMessages,
  defaultDialogMessages,
  defaultFormListMessages,
  defaultGetUploadResultUrl,
  defaultMarkdownEditorMessages,
  defaultMentionsMessages,
  defaultPasswordMessages,
  defaultPopconfirmMessages,
  defaultRateMessages,
  defaultRichTextEditorMessages,
  defaultSelectMessages,
  defaultSliderMessages,
  defaultTextareaMessages,
  defaultTransferMessages,
  defaultTreeSelectMessages,
  defaultUploadMessages,
  Descriptions,
  destroyNotifications,
  Dialog,
  Divider,
  Drawer,
  Dropdown,
  Empty,
  fileMatchesAccept,
  Form,
  formatUploadFileSize,
  FormItem,
  FormList,
  getBrowserBodyPortalHost,
  getConfig,
  getDaysInMonth,
  Grid,
  GridItem,
  Hero,
  Icon,
  IconActivity,
  IconAlertCircle,
  IconAlertTriangle,
  IconAlignCenter,
  IconAlignLeft,
  IconAlignRight,
  IconArchive,
  IconArrowDown,
  IconArrowDownLeft,
  IconArrowDownRight,
  IconArrowLeft,
  IconArrowLeftRight,
  IconArrowRight,
  IconArrowUp,
  IconArrowUpLeft,
  IconArrowUpRight,
  IconAtSign,
  IconAward,
  IconBadgeCheck,
  IconBan,
  IconBanknote,
  IconBarChart,
  IconBattery,
  IconBatteryCharging,
  IconBell,
  IconBookmark,
  IconBookOpen,
  IconBox,
  IconBraces,
  IconBrandAcrobat,
  IconBrandAdobexd,
  IconBrandAfterEffects,
  IconBrandAlipay,
  IconBrandAndroid,
  IconBrandApple,
  IconBrandBehance,
  IconBrandBinance,
  IconBrandChrome,
  IconBrandDingtalk,
  IconBrandDiscord,
  IconBrandDocker,
  IconBrandDropbox,
  IconBrandFacebook,
  IconBrandFigma,
  IconBrandFirefox,
  IconBrandGithub,
  IconBrandGitlab,
  IconBrandGoogle,
  IconBrandIllustrator,
  IconBrandIndesign,
  IconBrandInstagram,
  IconBrandLightroom,
  IconBrandLine,
  IconBrandLinkedin,
  IconBrandMedium,
  IconBrandMicrosoft,
  IconBrandNetflix,
  IconBrandNotion,
  IconBrandNpm,
  IconBrandPaypal,
  IconBrandPhotoshop,
  IconBrandPinterest,
  IconBrandPremiere,
  IconBrandQq,
  IconBrandReddit,
  IconBrandSignal,
  IconBrandSkype,
  IconBrandSlack,
  IconBrandSnapchat,
  IconBrandSpotify,
  IconBrandStripe,
  IconBrandTelegram,
  IconBrandTiktok,
  IconBrandTrello,
  IconBrandTwitch,
  IconBrandTwitter,
  IconBrandVercel,
  IconBrandWechat,
  IconBrandWeibo,
  IconBrandWhatsapp,
  IconBrandYoutube,
  IconBrandZoom,
  IconBriefcase,
  IconBug,
  IconBuilding,
  IconCalculator,
  IconCalendar,
  IconCamera,
  IconCheck,
  IconCheckCircle,
  IconChevronDown,
  IconChevronLeft,
  IconChevronRight,
  IconChevronUp,
  IconCircle,
  IconCircleDollarSign,
  IconCircleDot,
  IconCircleSplit,
  IconClipboard,
  IconClipboardCheck,
  IconClock,
  IconClose,
  IconCloud,
  IconCloudRain,
  IconCode,
  IconCoffee,
  IconCoins,
  IconColumns,
  IconCompass,
  IconCopy,
  IconCpu,
  IconCreditCard,
  IconCrown,
  IconDatabase,
  IconDisc,
  IconDollarSign,
  IconDownload,
  IconDroplet,
  IconEdit,
  IconExitFullscreen,
  IconExternalLink,
  IconEye,
  IconEyeOff,
  IconFile,
  IconFileCheck,
  IconFilePlus,
  IconFileText,
  IconFileX,
  IconFilm,
  IconFilter,
  IconFingerprint,
  IconFlag,
  IconFlame,
  IconFolder,
  IconFolderOpen,
  IconForward,
  IconGem,
  IconGift,
  IconGitBranch,
  IconGlobe,
  IconGraduationCap,
  IconGrid,
  IconGripVertical,
  IconHandshake,
  IconHash,
  IconHeadphones,
  IconHeart,
  IconHeartPulse,
  IconHelpCircle,
  IconHistory,
  IconHome,
  IconImage,
  IconImagePlus,
  IconInbox,
  IconInfo,
  IconKanban,
  IconKey,
  IconLamp,
  IconLandmark,
  IconLayers,
  IconLibrary,
  IconLink,
  IconLink2,
  IconList,
  IconLoader,
  IconLoader2,
  IconLock,
  IconLogIn,
  IconLogOut,
  IconMail,
  IconMailOpen,
  IconMapPin,
  IconMaximize2,
  IconMegaphone,
  IconMenu,
  IconMessageCircle,
  IconMessageSquare,
  IconMic,
  IconMicOff,
  IconMinus,
  IconMinusCircle,
  IconMonitorSplit,
  IconMoon,
  IconMoreHorizontal,
  IconMoreVertical,
  IconMusic,
  IconNavigation,
  IconNewspaper,
  IconNotebookPen,
  IconPackage,
  IconPalette,
  IconPaperClip,
  IconParkingCircle,
  IconPause,
  IconPercent,
  IconPhone,
  IconPieChart,
  IconPiggyBank,
  IconPill,
  IconPin,
  IconPlane,
  IconPlay,
  IconPlus,
  IconPlusCircle,
  IconPodcast,
  IconPower,
  IconPrinter,
  IconRadio,
  IconReceipt,
  IconRedo,
  IconRefresh,
  IconRepeat,
  IconReply,
  IconRotateCcw,
  IconRotateCw,
  IconRss,
  IconSave,
  IconScale,
  IconScan,
  IconScanBarcode,
  IconScissors,
  IconScrollText,
  IconSearch,
  IconSend,
  IconServer,
  IconSettings,
  IconShare,
  IconShield,
  IconShieldAlert,
  IconShieldCheck,
  IconShip,
  IconShoppingBag,
  IconShoppingCart,
  IconShuffle,
  IconSignature,
  IconSkipBack,
  IconSkipForward,
  IconSlidersHorizontal,
  IconSnowflake,
  IconSparkles,
  IconSquare,
  IconStamp,
  IconStar,
  IconStethoscope,
  IconStickyNote,
  IconStop,
  IconStore,
  IconSun,
  IconSyringe,
  IconTag,
  IconTarget,
  IconTerminal,
  IconTestTube,
  IconThermometer,
  IconThumbsDown,
  IconThumbsUp,
  IconTicket,
  IconTimer,
  IconTokenAda,
  IconTokenAlgo,
  IconTokenArb,
  IconTokenAtom,
  IconTokenAvax,
  IconTokenBnb,
  IconTokenBtc,
  IconTokenCro,
  IconTokenDai,
  IconTokenDoge,
  IconTokenDot,
  IconTokenEth,
  IconTokenFil,
  IconTokenFtm,
  IconTokenLink,
  IconTokenLtc,
  IconTokenMatic,
  IconTokenNear,
  IconTokenOp,
  IconTokenPepe,
  IconTokenShib,
  IconTokenSol,
  IconTokenSui,
  IconTokenTrx,
  IconTokenUni,
  IconTokenUsdc,
  IconTokenUsdt,
  IconTokenWbtc,
  IconTokenXrp,
  IconTrain,
  IconTrash,
  IconTrendingDown,
  IconTrendingUp,
  IconTruck,
  IconTv,
  IconType,
  IconUmbrella,
  IconUndo,
  IconUnlock,
  IconUpload,
  IconUser,
  IconUserCog,
  IconUserMinus,
  IconUserPlus,
  IconUsers,
  IconUtensils,
  IconVideo,
  IconVolume,
  IconVolumeOff,
  IconVote,
  IconWallet,
  IconWifi,
  IconWifiOff,
  IconWind,
  IconWine,
  IconWrench,
  IconXCircle,
  IconZap,
  IconZoomIn,
  IconZoomOut,
  Image,
  IMAGE_BUILTIN_FALLBACK_SRC,
  ImageViewer,
  initAnchorSpy,
  initDropdownEsc,
  Input,
  InputNumber,
  isMonthFullyOutsideMinMax,
  isYearFullyOutsideMinMax,
  Link,
  List,
  MarkdownEditor,
  Mentions,
  Menu,
  message,
  MessageContainer,
  Modal,
  MONTHS,
  MultiSelect,
  NavBar,
  notification,
  NotificationContainer,
  openNotification,
  PageHeader,
  Pagination,
  Paragraph,
  Password,
  PickerCalendarNav,
  Popconfirm,
  Popover,
  Progress,
  Radio,
  RadioGroup,
  Rate,
  readControlledOpenInput,
  readControlledStringInput,
  readMaybeSignal,
  removeMessage,
  removeToast,
  Result,
  RichTextEditor,
  runChunkedUpload,
  Search,
  Segmented,
  Select,
  setConfig,
  Sidebar,
  Skeleton,
  Slider,
  Spinner,
  Stack,
  Statistic,
  Steps,
  Switch,
  Table,
  Tabs,
  Tag,
  Text,
  Textarea,
  Timeline,
  TimePicker,
  Title,
  toast,
  ToastContainer,
  Tooltip,
  Transfer,
  Tree,
  TreeSelect,
  Upload,
  uploadActionWithPhase,
  uploadFilePhasedChunks,
  uploadFileSimple,
  yearGridPageStart,
} from "./desktop/mod.ts";
/**
 * 各国家/地区 1:1 国旗（`build-country-flags` 生成，`IconFlag`+两字母 ISO 大写，如 `IconFlagCN`）；与 ui-view 同源；根入口上列仅来自 desktop，故用 `export *` 再导出全部。
 */
export * from "./shared/basic/icons/flags/mod.ts";
export type {
  AccordionItem,
  AccordionProps,
  AffixProps,
  AlertProps,
  AlertType,
  AnchorLink,
  AnchorProps,
  AnchorSpyOptions,
  AutoCompleteMessages,
  AutoCompleteProps,
  AvatarProps,
  AvatarSize,
  BackTopProps,
  BackTopTarget,
  BadgeProps,
  BaseChartProps,
  BreadcrumbItem,
  BreadcrumbProps,
  ButtonGroupProps,
  ButtonProps,
  CalendarDaySelectionMode,
  CalendarMode,
  CalendarProps,
  CardProps,
  CarouselMessages,
  CarouselProps,
  CascaderAppearance,
  CascaderOption,
  CascaderProps,
  ChartBarProps,
  ChartBubbleProps,
  ChartData,
  ChartDoughnutProps,
  ChartLineProps,
  ChartOptions,
  ChartPieProps,
  ChartPolarAreaProps,
  ChartRadarProps,
  ChartScatterProps,
  ChartType,
  CheckboxGroupOption,
  CheckboxGroupProps,
  CheckboxProps,
  ChunkedUploadOptions,
  ChunkUploadContext,
  CodeBlockLanguage,
  CodeBlockProps,
  CollapseItem,
  CollapseProps,
  ColorPickerHandle,
  ColorPickerMessages,
  ColorPickerProps,
  CommentProps,
  ConfigProviderBatchConfig,
  ConfigProviderConfig,
  ConfigProviderProps,
  ContainerProps,
  ContainerSize,
  ControlledOpenInput,
  ControlledStringInput,
  DatePickerMode,
  DatePickerProps,
  DatePickerRangeValue,
  DatePickerValue,
  DateTimePickerMessages,
  DateTimePickerMode,
  DateTimePickerProps,
  DateTimePickerRangeValue,
  DateTimePickerValue,
  DescriptionsItem,
  DescriptionsProps,
  DialogMessages,
  DialogMobileLayout,
  DialogProps,
  DialogVariant,
  DividerOrientation,
  DividerProps,
  DrawerOpenInput,
  DrawerPlacement,
  DrawerProps,
  DrawerTitleAlign,
  DrawerTitleInput,
  DropdownPlacement,
  DropdownProps,
  EmptyProps,
  FormItemLabelAlign,
  FormItemLabelPosition,
  FormItemProps,
  FormLayout,
  FormListMessages,
  FormListProps,
  FormListRenderRowContext,
  FormProps,
  GridItemProps,
  GridProps,
  HeroLayout,
  HeroProps,
  IconComponentProps,
  IconProps,
  ImageProps,
  ImageViewerImageTransition,
  ImageViewerProps,
  ImageViewerTransition,
  ImageViewerTransitionInput,
  InputNumberProps,
  InputProps,
  LinkProps,
  ListItemProps,
  ListProps,
  MarkdownEditorMessages,
  MarkdownEditorPreviewMode,
  MarkdownEditorProps,
  MaybeSignal,
  MentionOption,
  MentionsMessages,
  MentionsProps,
  MenuItem,
  MenuProps,
  MessageItem,
  MessagePlacement,
  MessageType,
  ModalOpenInput,
  ModalPlacement,
  ModalProps,
  ModalTitleAlign,
  ModalTitleInput,
  ModalWidthInput,
  ModalWidthPrimitive,
  MultiSelectAppearance,
  MultiSelectOption,
  MultiSelectProps,
  NavBarProps,
  NotificationItem,
  NotificationPlacement,
  NotificationType,
  OpenOptions,
  PageHeaderProps,
  PaginationProps,
  ParagraphProps,
  PasswordMessages,
  PasswordProps,
  PickerCalendarHeaderPanel,
  PickerCalendarNavProps,
  PopconfirmMessages,
  PopconfirmOpenInput,
  PopconfirmPlacement,
  PopconfirmProps,
  PopoverPlacement,
  PopoverProps,
  ProgressPercentInput,
  ProgressProps,
  ProgressStatus,
  ProgressType,
  RadioGroupOption,
  RadioGroupProps,
  RadioProps,
  RateMessages,
  RateProps,
  ResultProps,
  ResultStatus,
  RichTextEditorMessages,
  RichTextEditorProps,
  SearchProps,
  SegmentedOption,
  SegmentedProps,
  SelectAppearance,
  SelectMessages,
  SelectOption,
  SelectProps,
  SidebarItem,
  SidebarProps,
  SidebarSubItem,
  SkeletonProps,
  SliderMessages,
  SliderProps,
  SortOrder,
  SpinnerProps,
  StackAlign,
  StackDirection,
  StackJustify,
  StackProps,
  StatisticProps,
  StepItem,
  StepsProps,
  StepStatus,
  SwitchProps,
  TabItem,
  TableCellChangePayload,
  TableColumn,
  TableColumnEditable,
  TableEditableOption,
  TableProps,
  TabsProps,
  TabsType,
  TagProps,
  TextareaMessages,
  TextareaProps,
  TextProps,
  ThemeMode,
  TimelineItemProps,
  TimelineProps,
  TimePickerMode,
  TimePickerProps,
  TimePickerRangeValue,
  TimePickerValue,
  TitleProps,
  ToastItem,
  ToastPlacement,
  ToastType,
  ToolbarConfig,
  ToolbarItem,
  ToolbarPreset,
  TooltipPlacement,
  TooltipProps,
  TransferItem,
  TransferMessages,
  TransferProps,
  TreeNode,
  TreeProps,
  TreeSelectAppearance,
  TreeSelectMessages,
  TreeSelectOption,
  TreeSelectProps,
  UploadCoreProps,
  UploadFile,
  UploadFileStatus,
  UploadMessages,
  UploadMultipleValueMode,
  UploadProps,
} from "./desktop/mod.ts";
