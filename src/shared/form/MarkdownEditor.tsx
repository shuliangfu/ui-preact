...
export interface MarkdownEditorMessages {
  /** 编辑区与切换按钮共用的「源码」label / tooltip */
  sourceLabel: string;
  /** 「预览」label / aria-label（预览区 region） */
  previewLabel: string;
  /** 渲染预览按钮 tooltip */
...
function takeMarkdownEditorActiveTab(
  ...
  if (trimmedId) {
    ...
    if (hit) return hit;
    const s = signal<"edit" | "preview">("edit");
    mdEditorActiveTabById.set(k, s);
    return s;
  }
  const v = props.value;
  if (v !== undefined && v instanceof SignalCtor) {
  ...
  return signal<"edit" | "preview">("edit");
}
 ...
 * @param msg - 合并后的用户文案（空内容 / 解析失败占位）
 */
function safeParseMarkdown(
  md: string,
  opts: ParseOptions,
  msg: MarkdownEditorMessages,
): string {
  if (md.trim() === "") {
    return msg.previewEmpty;
  }
  try {
    return parse(md, opts);
  } catch {
    return msg.previewError;
  }
}
...
export function MarkdownEditor(props: MarkdownEditorProps): JSX.Element {
  ...
  /** 合并默认中文文案与外部传入 messages（预览解析等闭包内再次合并以响应 props.messages 变化） */
  const m = { ...defaultMarkdownEditorMessages, ...props.messages };
  /** textarea `aria-label`：`ariaLabel` prop 优先 */
  const textareaAriaResolved = ariaLabel ?? m.textareaAriaLabel;

  /** 全屏时用于切换 `md-editor-fullscreen` 的根容器 */
  const rootRef = useRef<HTMLDivElement>(null);
  /** 与 RTE `rteFullscreen` 同理：同步全屏态以切换图标与 Tooltip */
  const mdFullscreen = signal(false);
  ...
  const mergedParseOptions = computed((): ParseOptions => ({
    ...DEFAULT_PARSE,
    ...parseOptionsProp,
  }));

  /**
   * 追踪受控 `value`（含 Preact Signal）变化以驱动预览 HTML。
   */
  const markdownSource = computed(() => readMaybeSignal(props.value) ?? "");

  const previewHtml = computed(() => {
    if (preview === "off") return "";
    const msg = { ...defaultMarkdownEditorMessages, ...props.messages };
    return safeParseMarkdown(
  ...
  effect(() => {
    ...
    const len = markdownSource.value.length;
    const remaining = Math.max(0, maxLength - len);
    const badge = charCountRef.current;
    if (badge != null) {
      const msg = { ...defaultMarkdownEditorMessages, ...props.messages };
      badge.textContent = msg.charCount(remaining, maxLength);
    }
  });
  ...
  const textareaNode = (
    <textarea
      ...
      name={name}
      rows={rows}
      value={readMaybeSignal(props.value) ?? ""}
      ...
      autocapitalize="off"
      autocomplete="off"
      aria-label={textareaAriaResolved}
      ...
      {showToolbar && (
        ...
        >
          ...
          {allowFullscreen && (
            ...
            >
              <MdToolbarItemTip content={fs ? m.fullscreenExit : m.fullscreen}>
                <button
                  type="button"
                  class={twMerge(
                    toolbarBtnBase,
