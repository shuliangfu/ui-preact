/**
 * @fileoverview 文档路由 `/desktop` 的浏览器测试。
 * 桌面文档首页 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop";

describe("文档页 E2E：/desktop（桌面文档首页）", () => {
  ...
  it("本页关键词命中且 main 内完成浅层交互探针", async (t) => {
    ...
    await runKeywordAndShallowHere(t, env, DOC_PATH, [
      /桌面|组件|文档|Dreamer|Overview|预览/i,
    ]);
  }, DOCS_BROWSER_CONFIG);

  /**
   * 文档小节「Tailwind 按需插件」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·Tailwind 按需插件", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "Tailwind 按需插件";
  ...
  /**
   * 文档小节「提示」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·提示", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
