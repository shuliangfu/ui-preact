/**
 * @fileoverview 文档路由 `/desktop/data-display/calendar` 的浏览器测试。
 * Calendar 日历 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/data-display/calendar";

describe("文档页 E2E：/desktop/data-display/calendar（Calendar 日历）", () => {
  ...
  /**
   * 文档小节「基础选日」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·基础选日", async (t) => {
    if (!t?.browser?.goto) return;
  ...
  /**
   * 文档小节「mode=year 年视图」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·mode=year 年视图", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "mode=year 年视图";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「fullscreen」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·fullscreen", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "fullscreen";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
