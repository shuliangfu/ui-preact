/**
 * @fileoverview 文档路由 `/desktop/data-display/carousel` 的浏览器测试。
 * Carousel 轮播 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/data-display/carousel";

describe("文档页 E2E：/desktop/data-display/carousel（Carousel 轮播）", () => {
  ...
  /**
   * 文档小节「dots + arrows（网络图片，高度可设置）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·dots + arrows（网络图片，高度可设置）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "dots + arrows（网络图片，高度可设置）";
  ...
  /**
   * 文档小节「autoplay」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·autoplay", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "autoplay";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「切换效果 effect（slide / fade / zoom / flip）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·切换效果 effect（slide / fade / zoom / flip）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      /** 与文档 `Title` 全文一致（含 mosaic / random），避免 `flip）` 与 `flip / mosaic` 断在 includes 边界 */
      const needle =
