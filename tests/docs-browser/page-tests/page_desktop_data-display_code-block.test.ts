/**
 * @fileoverview 文档路由 `/desktop/data-display/code-block` 的浏览器测试。
 * CodeBlock 代码块 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/data-display/code-block";

describe("文档页 E2E：/desktop/data-display/code-block（CodeBlock 代码块）", () => {
  ...
  /**
   * 文档小节「title + copyable + onCopy」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·title + copyable + onCopy", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "title + copyable + onCopy";
  ...
  /**
   * 文档小节「showLineNumbers + maxHeight」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·showLineNumbers + maxHeight", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "showLineNumbers + maxHeight";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「language=typescript / plaintext」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·language=typescript / plaintext", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "language=typescript / plaintext";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
