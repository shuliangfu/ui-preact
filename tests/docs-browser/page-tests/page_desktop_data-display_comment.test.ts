/**
 * @fileoverview 文档路由 `/desktop/data-display/comment` 的浏览器测试。
 * Comment 评论 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/data-display/comment";

describe("文档页 E2E：/desktop/data-display/comment（Comment 评论）", () => {
  ...
  /**
   * 文档小节「基础（author / avatar / datetime / actions）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·基础（author / avatar / datetime / actions）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "基础（author / avatar / datetime / actions）";
  ...
  /**
   * 文档小节「replies 嵌套回复」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·replies 嵌套回复", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "replies 嵌套回复";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
