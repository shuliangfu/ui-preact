/**
 * @fileoverview 文档路由 `/desktop/data-display/image-viewer` 的浏览器测试。
 * ImageViewer 图片预览 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/data-display/image-viewer";

describe("文档页 E2E：/desktop/data-display/image-viewer（ImageViewer 图片预览）", () => {
  ...
  it("本页关键词命中且 main 内完成浅层交互探针", async (t) => {
    ...
    await runKeywordAndShallowHere(t, env, DOC_PATH, [
      /ImageViewer|图片预览|预览/i,
    ]);
  }, DOCS_BROWSER_CONFIG);

  /**
   * 文档小节「多图切换 + 缩略图」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·多图切换 + 缩略图", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "多图切换 + 缩略图";
