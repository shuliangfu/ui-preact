/**
 * @fileoverview 文档路由 `/desktop/data-display/image` 的浏览器测试。
 * Image 图片 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/data-display/image";

describe("文档页 E2E：/desktop/data-display/image（Image 图片）", () => {
  ...
  /**
   * 文档小节「基础 + fit + preview + rounded」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·基础 + fit + preview + rounded", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "基础 + fit + preview + rounded";
  ...
  /**
   * 文档小节「fallback / lazy」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·fallback / lazy", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "fallback / lazy";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
