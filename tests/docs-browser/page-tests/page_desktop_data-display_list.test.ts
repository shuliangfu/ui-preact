/**
 * @fileoverview 文档路由 `/desktop/data-display/list` 的浏览器测试。
 * List 列表 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
describe("文档页 E2E：/desktop/data-display/list（List 列表）", () => {
  ...
  /**
   * 文档小节「header / footer / bordered」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  ...
  /**
   * 文档小节「split=false / loading」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  ...
  /**
   * 文档小节「grid（多列 + 响应式）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
