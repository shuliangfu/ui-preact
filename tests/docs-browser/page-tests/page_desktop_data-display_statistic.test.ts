/**
 * @fileoverview 文档路由 `/desktop/data-display/statistic` 的浏览器测试。
 * Statistic 统计 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/data-display/statistic";

describe("文档页 E2E：/desktop/data-display/statistic（Statistic 统计）", () => {
  ...
  /**
   * 文档小节「prefix / suffix / precision / groupSeparator」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·prefix / suffix / precision / groupSeparator", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "prefix / suffix / precision / groupSeparator";
  ...
  /**
   * 文档小节「趋势箭头（数字正负自动判断）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·趋势箭头（数字正负自动判断）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "趋势箭头（数字正负自动判断）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
