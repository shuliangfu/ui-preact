/**
 * @fileoverview 文档路由 `/desktop/feedback/alert` 的浏览器测试。
 * Alert 警告 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/feedback/alert";

describe("文档页 E2E：/desktop/feedback/alert（Alert 警告）", () => {
  ...
  /**
   * 文档小节「带描述」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·带描述", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
  ...
  /**
   * 文档小节「可关闭」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·可关闭", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
  ...
  /**
   * 文档小节「自定义操作」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·自定义操作", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
  ...
  /**
   * 文档小节「不显示图标（showIcon=false）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·不显示图标（showIcon=false）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "不显示图标（showIcon=false）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「横幅样式（banner）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·横幅样式（banner）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "横幅样式（banner）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
