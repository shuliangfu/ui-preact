/**
 * @fileoverview 文档路由 `/desktop/feedback/dialog` 的浏览器测试。
 * Dialog 对话框 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/feedback/dialog";

describe("文档页 E2E：/desktop/feedback/dialog（Dialog 对话框）", () => {
  ...
  /**
   * 打开 Dialog 后点「确定」或「取消」关闭。
   */
  it("可打开 Dialog 并关闭", async (t) => {
    if (!t?.browser?.goto) return;
    ...
    const openOk = await t.browser.evaluate(() => {
      ...
      for (let i = 0; i < btns.length; i++) {
        ...
        if ((b.textContent || "").includes("打开 Dialog")) {
          b.click();
          return true;
        }
      }
    ...
    await env.delay(200);
    const closeOk = await t.browser.evaluate(() => {
  ...
  /**
   * 文档小节「警告确认（warning）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·警告确认（warning）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "警告确认（warning）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「危险操作（danger）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·危险操作（danger）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "危险操作（danger）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「确定 Loading（confirmLoading）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·确定 Loading（confirmLoading）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "确定 Loading（confirmLoading）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
