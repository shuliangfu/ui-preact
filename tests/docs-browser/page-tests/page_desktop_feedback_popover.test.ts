/**
 * @fileoverview 文档路由 `/desktop/feedback/popover` 的浏览器测试。
 * Popover 气泡卡片 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/feedback/popover";

describe("文档页 E2E：/desktop/feedback/popover（Popover 气泡卡片）", () => {
  ...
  /**
   * 文档小节「带标题与四向位置」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·带标题与四向位置", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "带标题与四向位置";
