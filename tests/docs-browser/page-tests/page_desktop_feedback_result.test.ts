/**
 * @fileoverview 文档路由 `/desktop/feedback/result` 的浏览器测试。
 * Result 结果页 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/feedback/result";

describe("文档页 E2E：/desktop/feedback/result（Result 结果页）", () => {
  ...
  /**
   * 文档小节「成功」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·成功", async (t) => {
    if (!t?.browser?.goto) return;
  ...
  /**
   * 文档小节「失败」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·失败", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
  ...
  /**
   * 文档小节「信息 / 警告」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·信息 / 警告", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
  ...
  /**
   * 文档小节「403 / 404」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·403 / 404", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "403 / 404";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
