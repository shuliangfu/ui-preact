/**
 * @fileoverview 文档路由 `/desktop/feedback/tooltip` 的浏览器测试。
 * Tooltip 文字提示 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/feedback/tooltip";

describe("文档页 E2E：/desktop/feedback/tooltip（Tooltip 文字提示）", () => {
  ...
  /**
   * 文档小节「位置（placement）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·位置（placement）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "位置（placement）";
  ...
  /**
   * 文档小节「长文案」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·长文案", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
  ...
  /**
   * 文档小节「无箭头（arrow=false）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·无箭头（arrow=false）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "无箭头（arrow=false）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
