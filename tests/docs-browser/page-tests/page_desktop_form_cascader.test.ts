/**
 * @fileoverview 文档路由 `/desktop/form/cascader` 的浏览器测试。
 * Cascader 级联 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
describe("文档页 E2E：/desktop/form/cascader（Cascader 级联）", () => {
  ...
  /**
   * 级联：打开面板后依次点「浙江」「杭州」「西湖区」（与文档示例数据一致）。
   */
  it("级联选择器可展开并完成三级点选", async (t) => {
    ...
    const step1 = await t.browser.evaluate(() => {
      ...
      const buttons = panel.querySelectorAll('button[type="button"]');
      for (let i = 0; i < buttons.length; i++) {
        const b = buttons[i] as HTMLButtonElement;
        if (b.disabled) continue;
        if ((b.textContent || "").trim() === "浙江") {
          b.click();
          return true;
        }
      }
  ...
  /**
   * 文档小节「静态数据（多级嵌套）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  ...
  /**
   * 文档小节「静态数据（二级：市 / 区）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  ...
  /**
   * 文档小节「表单 name（多段隐藏域）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  ...
  /**
   * 文档小节「动态加载（loadChildren）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
