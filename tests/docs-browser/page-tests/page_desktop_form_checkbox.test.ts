/**
 * @fileoverview 文档路由 `/desktop/form/checkbox` 的浏览器测试。
 * Checkbox 复选框 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/checkbox";

describe("文档页 E2E：/desktop/form/checkbox（Checkbox 复选框）", () => {
  ...
  /**
   * 连点两次首个 checkbox，勾选状态应发生变化再变回。
   */
  it("首个 checkbox 可切换两次", async (t) => {
    if (!t?.browser?.goto) return;
  ...
  /**
   * 文档小节「name / value / id」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·name / value / id", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "name / value / id";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
