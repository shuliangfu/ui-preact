/**
 * @fileoverview 文档路由 `/desktop/form/form-containers` 的浏览器测试。
 * Form 容器 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/form-containers";

describe("文档页 E2E：/desktop/form/form-containers（Form 容器）", () => {
  ...
  /**
   * 表单容器页内示例 input：写入、blur，验证可交互。
   */
  it("main 内示例输入可输入并在 blur 后失焦", async (t) => {
    if (!t?.browser?.goto) return;
    ...
    const notSameFocused = await t.browser.evaluate(() => {
      ...
      return (ae as HTMLInputElement).value !== "e2e-form-container";
    }) as boolean;
    expect(notSameFocused).toBe(true);
  }, DOCS_BROWSER_CONFIG);

  /**
   * 文档小节「Form：layout」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·Form：layout", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "Form：layout";
      const main = document.querySelector("main");
      if (!main) return false;
  ...
  /**
   * 文档小节「FormItem：标签位置与必填」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·FormItem：标签位置与必填", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "FormItem：标签位置与必填";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「FormList：动态增减行」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·FormList：动态增减行", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "FormList：动态增减行";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「Form」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·Form", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
  ...
  /**
   * 文档小节「FormItem」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·FormItem", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "FormItem";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「FormList」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·FormList", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "FormList";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
