/**
 * @fileoverview 文档路由 `/desktop/form/input-number` 的浏览器测试。
 * InputNumber 数字 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/input-number";

describe("文档页 E2E：/desktop/form/input-number（InputNumber 数字）", () => {
  ...
  /**
   * 数字输入：改 value、派发 input、blur，并断言焦点离开该输入框。
   */
  it("main 内数字输入可改值并在 blur 后失焦", async (t) => {
    if (!t?.browser?.goto) return;
    ...
    const notSameFocused = await t.browser.evaluate(() => {
      ...
      if (!ae) return true;
      if (ae.tagName !== "INPUT") return true;
      return (ae as HTMLInputElement).value !== "42";
    }) as boolean;
    expect(notSameFocused).toBe(true);
  }, DOCS_BROWSER_CONFIG);

  /**
   * 文档小节「步进按钮 + min/max/step」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·步进按钮 + min/max/step", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "步进按钮 + min/max/step";
      const main = document.querySelector("main");
      if (!main) return false;
  ...
  /**
   * 文档小节「小数步进（step=0.1）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·小数步进（step=0.1）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "小数步进（step=0.1）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「placeholder」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·placeholder", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
