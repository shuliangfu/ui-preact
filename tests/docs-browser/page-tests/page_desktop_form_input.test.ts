/**
 * @fileoverview 文档路由 `/desktop/form/input` 的浏览器测试。
 * Input 输入框 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
describe("文档页 E2E：/desktop/form/input（Input 输入框）", () => {
  ...
  /**
   * 文档小节「required」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  ...
  /**
   * 文档小节「FormItem：hideRequiredMark」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  ...
  /**
   * 文档小节「error」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·error", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
  ...
  /**
   * 文档小节「readOnly」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  ...
  /**
   * 文档小节「allowClear 可清除」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  ...
  /**
   * 文档小节「标签在左侧」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·标签在左侧", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
  ...
  /**
   * 文档小节「左对齐」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·左对齐", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
  ...
  /**
   * 文档小节「右对齐」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·右对齐", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
  ...
  /**
   * 文档小节「type / prefix / suffix」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
