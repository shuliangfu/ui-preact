/**
 * @fileoverview 文档路由 `/desktop/form/markdown-editor` 的浏览器测试。
 * Markdown 编辑器 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/markdown-editor";

describe("文档页 E2E：/desktop/form/markdown-editor（Markdown 编辑器）", () => {
  ...
  /**
   * Markdown 编辑区为 textarea：写入并失焦。
   */
  it("main 内 Markdown textarea 可输入", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(500);
    const ok = await t.browser.evaluate(() => {
      const main = document.querySelector("main");
      const ta = main?.querySelector("textarea") as HTMLTextAreaElement | null;
  ...
  /**
   * 文档小节「preview：split（默认）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·preview：split（默认）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "preview：split（默认）";
      const main = document.querySelector("main");
  ...
  /**
   * 文档小节「preview：tabs」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·preview：tabs", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "preview：tabs";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「preview：off」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·preview：off", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "preview：off";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「maxLength」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·maxLength", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "maxLength";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
