/**
 * @fileoverview 文档路由 `/desktop/form/mentions` 的浏览器测试。
 * Mentions 提及 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/mentions";

describe("文档页 E2E：/desktop/form/mentions（Mentions 提及）", () => {
  ...
  /**
   * Mentions 基于 input：写入文本、blur，验证可编辑。
   */
  it("main 内提及输入框可输入并失焦", async (t) => {
    if (!t?.browser?.goto) return;
    ...
    const typedOk = await t.browser.evaluate(() => {
      ...
      /** Mentions 为 textarea；优先示例占位「输入 @ 提及」 */
      const el = main.querySelector<HTMLTextAreaElement>(
        'textarea[placeholder="输入 @ 提及"]',
      ) || main.querySelector<HTMLTextAreaElement>("textarea:not(:disabled)");
  ...
  /**
   * 文档小节「带候选下拉（输入 @ 触发）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·带候选下拉（输入 @ 触发）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "带候选下拉（输入 @ 触发）";
      const main = document.querySelector("main");
      if (!main) return false;
  ...
  /**
   * 文档小节「有默认值 / 无候选（仅多行输入）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·有默认值 / 无候选（仅多行输入）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "有默认值 / 无候选（仅多行输入）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
