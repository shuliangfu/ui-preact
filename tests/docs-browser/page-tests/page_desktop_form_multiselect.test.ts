/**
 * @fileoverview 文档路由 `/desktop/form/multiselect` 的浏览器测试。
 * MultiSelect 多选 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/multiselect";

describe("文档页 E2E：/desktop/form/multiselect（MultiSelect 多选）", () => {
  ...
  /**
   * 多选：展开 listbox，点选一项「选项 A」（与文档示例 options 一致）。
   */
  it("多选下拉可展开并勾选「选项 A」", async (t) => {
    if (!t?.browser?.goto) return;
  ...
  /**
   * 文档小节「全选 / 清空」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·全选 / 清空", async (t) => {
    if (!t?.browser?.goto) return;
    await sharedEnv.goto(t, DOC_PATH);
  ...
  /**
   * 文档小节「有默认选中 + 选项含 disabled」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·有默认选中 + 选项含 disabled", async (t) => {
    if (!t?.browser?.goto) return;
    await sharedEnv.goto(t, DOC_PATH);
    await sharedEnv.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "有默认选中 + 选项含 disabled";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
