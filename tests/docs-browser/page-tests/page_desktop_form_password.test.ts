/**
 * @fileoverview 文档路由 `/desktop/form/password` 的浏览器测试。
 * Password 密码 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/password";

describe("文档页 E2E：/desktop/form/password（Password 密码）", () => {
   ...
   */
  it("main 内密码框可输入并在 blur 后失焦", async (t) => {
    if (!t?.browser?.goto) return;
  ...
  /**
   * 文档小节「带强度提示（showStrength）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·带强度提示（showStrength）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "带强度提示（showStrength）";
      const main = document.querySelector("main");
      if (!main) return false;
