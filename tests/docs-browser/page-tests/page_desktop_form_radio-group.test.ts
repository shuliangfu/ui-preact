/**
 * @fileoverview 文档路由 `/desktop/form/radio-group` 的浏览器测试。
 * RadioGroup 单选组 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/radio-group";

describe("文档页 E2E：/desktop/form/radio-group（RadioGroup 单选组）", () => {
  ...
  /**
   * 点击第二个 radio（若存在），验证可选中另一项。
   */
  it("单选组可点选另一项 radio", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(500);
    const ok = await t.browser.evaluate(() => {
      const main = document.querySelector("main");
      const radios = main?.querySelectorAll<HTMLInputElement>(
        'input[type="radio"]:not([disabled])',
      );
      if (!radios || radios.length < 2) return false;
      radios[1].click();
      return radios[1].checked === true;
    }) as boolean;
    expect(ok).toBe(true);
  }, DOCS_BROWSER_CONFIG);
  ...
  /**
   * 文档小节「有默认值 + 选项含 disabled」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·有默认值 + 选项含 disabled", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "有默认值 + 选项含 disabled";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「direction=horizontal（横向）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·direction=horizontal（横向）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "direction=horizontal（横向）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「direction=vertical（纵向）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·direction=vertical（纵向）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "direction=vertical（纵向）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「error / disabled」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·error / disabled", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "error / disabled";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
