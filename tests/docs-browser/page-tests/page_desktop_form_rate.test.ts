/**
 * @fileoverview 文档路由 `/desktop/form/rate` 的浏览器测试。
 * Rate 评分 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/rate";

describe("文档页 E2E：/desktop/form/rate（Rate 评分）", () => {
  ...
  /**
   * 点击「3 星」评分按钮（与 Rate 组件 aria-label 约定一致）。
   */
  it("评分可点击第三颗星", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(500);
    const ok = await t.browser.evaluate(() => {
      const main = document.querySelector("main");
      const star = main?.querySelector(
  ...
  /**
   * 文档小节「基础（5 星）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·基础（5 星）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
  ...
  /**
   * 文档小节「allowHalf 半星」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·allowHalf 半星", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "allowHalf 半星";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「count / disabled」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·count / disabled", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "count / disabled";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
