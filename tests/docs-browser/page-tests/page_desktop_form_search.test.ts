/**
 * @fileoverview 文档路由 `/desktop/form/search` 的浏览器测试。
 * Search 搜索 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/search";

describe("文档页 E2E：/desktop/form/search（Search 搜索）", () => {
  ...
  /**
   * 在 main 内找首个可写 input/textarea：写入、派发 input、blur，并断言焦点不再留在该控件上。
   */
  it("main 内搜索框可输入并在 blur 后失焦", async (t) => {
    if (!t?.browser?.goto) return;
    ...
    const notSameFocused = await t.browser.evaluate(() => {
      ...
      return (ae as HTMLInputElement).value !== "e2e-blur-test";
    }) as boolean;
    expect(notSameFocused).toBe(true);
  }, DOCS_BROWSER_CONFIG);

  /**
   * 文档小节「基础（带 onSearch 时显示搜索与清除）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·基础（带 onSearch 时显示搜索与清除）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "基础（带 onSearch 时显示搜索与清除）";
      const main = document.querySelector("main");
      if (!main) return false;
  ...
  /**
   * 文档小节「仅输入（无 onSearch，无搜索/清除按钮）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·仅输入（无 onSearch，无搜索/清除按钮）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "仅输入（无 onSearch，无搜索/清除按钮）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「失焦再请求（onBlur）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·失焦再请求（onBlur）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "失焦再请求（onBlur）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「size」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·size", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
