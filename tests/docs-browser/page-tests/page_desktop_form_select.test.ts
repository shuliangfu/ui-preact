/**
 * @fileoverview 文档路由 `/desktop/form/select` 的浏览器测试。
 * Select 选择器 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/select";

describe("文档页 E2E：/desktop/form/select（Select 选择器）", () => {
  ...
  /**
   * 点开 listbox 触发器，在可见下拉中选中文案含「选项 A」的 option。
   */
  it("单选下拉可展开并选「选项 A」", async (t) => {
    if (!t?.browser?.goto) return;
    ...
    const picked = await t.browser.evaluate(() => {
      ...
      for (let i = 0; i < lbs.length; i++) {
        ...
        for (let j = 0; j < opts.length; j++) {
          ...
          if ((o as HTMLButtonElement).disabled) continue;
          if ((o.textContent || "").includes("选项 A")) {
            o.click();
            return true;
          }
        }
