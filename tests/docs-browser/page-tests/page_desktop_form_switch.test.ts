/**
 * @fileoverview 文档路由 `/desktop/form/switch` 的浏览器测试。
 * Switch 开关 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/switch";

describe("文档页 E2E：/desktop/form/switch（Switch 开关）", () => {
  ...
  /**
   * 点击 main 内第一个 switch 控件（role=switch 或常见开关按钮）。
   */
  it("开关可点击切换", async (t) => {
    if (!t?.browser?.goto) return;
    ...
    const ok = await t.browser.evaluate(() => {
      ...
      /** Switch 实现为 `label` + 隐藏 `input.peer[type=checkbox]`，无 role=switch */
      const sw =
        (main.querySelector('[role="switch"]') as HTMLElement | null) ||
        (main.querySelector(
          "button[aria-checked]",
        ) as HTMLElement | null) ||
        (main.querySelector(
  ...
  /**
   * 文档小节「checkedChildren / unCheckedChildren」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·checkedChildren / unCheckedChildren", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "checkedChildren / unCheckedChildren";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「name / id」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·name / id", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "name / id";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
