/**
 * @fileoverview 文档路由 `/desktop/form/transfer` 的浏览器测试。
 * Transfer 穿梭框 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/transfer";

describe("文档页 E2E：/desktop/form/transfer（Transfer 穿梭框）", () => {
  ...
  /**
   * 点选待选列表中含「苹果」的项，再点「右移」穿梭到已选。
   */
  it("穿梭框可选中「苹果」并右移", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(500);
    const pickApple = await t.browser.evaluate(() => {
    ...
    expect(pickApple).toBe(true);
    await env.delay(120);
    const moveRight = await t.browser.evaluate(() => {
      const b = document.querySelector(
        'button[aria-label="右移"]',
      ) as HTMLButtonElement | null;
      if (!b || b.disabled) return false;
      b.click();
      return true;
    }) as boolean;
    expect(moveRight).toBe(true);
  }, DOCS_BROWSER_CONFIG);

  /**
   * 文档小节「基础（无搜索）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·基础（无搜索）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
  ...
  /**
   * 文档小节「showSearch 搜索筛选」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·showSearch 搜索筛选", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "showSearch 搜索筛选";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「dataSource 含 disabled 项 / 整组 disabled」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·dataSource 含 disabled 项 / 整组 disabled", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "dataSource 含 disabled 项 / 整组 disabled";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
