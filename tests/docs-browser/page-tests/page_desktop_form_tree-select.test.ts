/**
 * @fileoverview 文档路由 `/desktop/form/tree-select` 的浏览器测试。
 * TreeSelect 树选择 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/tree-select";

describe("文档页 E2E：/desktop/form/tree-select（TreeSelect 树选择）", () => {
  ...
  /**
   * 树选择：展开 listbox，在浮层中点选叶子「前端」。
   */
  it("树选择可展开并选「前端」", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(500);
    const opened = await t.browser.evaluate(() => {
      const btn = document.querySelector(
        'main button[aria-haspopup="listbox"]:not([disabled])',
      );
      if (!btn) return false;
    ...
    const picked = await t.browser.evaluate(() => {
      ...
      for (let i = 0; i < lbs.length; i++) {
        ...
        const nodes = lb.querySelectorAll(
          '[role="option"], [role="treeitem"], button[type="button"]',
        );
        for (let j = 0; j < nodes.length; j++) {
          const n = nodes[j] as HTMLElement;
          if ((n as HTMLButtonElement).disabled) continue;
          if ((n.textContent || "").trim() === "前端") {
            n.click();
            return true;
          }
        }
  ...
  /**
   * 文档小节「多层级（缩进）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·多层级（缩进）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
  ...
  /**
   * 文档小节「name / id（表单）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·name / id（表单）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "name / id（表单）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
