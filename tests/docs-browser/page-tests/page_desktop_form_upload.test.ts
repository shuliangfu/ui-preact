/**
 * @fileoverview 文档路由 `/desktop/form/upload` 的浏览器测试。
 * Upload 上传 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/form/upload";

describe("文档页 E2E：/desktop/form/upload（Upload 上传）", () => {
  ...
  /**
   * 上传示例区应存在可用的 file input（不真正选文件，只断言可挂载）。
   */
  it("main 内存在可用的 file 输入控件", async (t) => {
    if (!t?.browser?.goto) return;
  ...
  /**
   * 文档小节「action：后台演示 API」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·action：后台演示 API", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "action：后台演示 API";
      const main = document.querySelector("main");
  ...
  /**
   * 文档小节「action + 分片（与 @dreamer/upload/server 一致）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·action + 分片（与 @dreamer/upload/server 一致）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "action + 分片（与 @dreamer/upload/server 一致）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「多选 + 关闭拖拽区（IconUpload + 上传图片）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·多选 + 关闭拖拽区（IconUpload + 上传图片）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "多选 + 关闭拖拽区（IconUpload + 上传图片）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
