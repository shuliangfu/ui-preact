/**
 * @fileoverview 文档路由 `/desktop/layout/accordion` 的浏览器测试。
 * Accordion 手风琴 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/layout/accordion";

describe("文档页 E2E：/desktop/layout/accordion（Accordion 手风琴）", () => {
  ...
  /**
   * 文档小节「allowMultiple=true」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·allowMultiple=true", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "allowMultiple=true";
  ...
  /**
   * 文档小节「allowMultiple=false（单选）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·allowMultiple=false（单选）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "allowMultiple=false（单选）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
