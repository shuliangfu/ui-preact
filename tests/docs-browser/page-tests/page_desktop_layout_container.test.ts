/**
 * @fileoverview 文档路由 `/desktop/layout/container` 的浏览器测试。
 * Container 容器 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/layout/container";

describe("文档页 E2E：/desktop/layout/container（Container 容器）", () => {
  ...
  /**
   * 文档小节「maxWidth（md / xl）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·maxWidth（md / xl）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "maxWidth（md / xl）";
  ...
  /**
   * 文档小节「centered=false / padded=false」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·centered=false / padded=false", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "centered=false / padded=false";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
