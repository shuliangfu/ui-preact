/**
 * @fileoverview 文档路由 `/desktop/layout/hero` 的浏览器测试。
 * Hero 首屏 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/layout/hero";

describe("文档页 E2E：/desktop/layout/hero（Hero 首屏）", () => {
  ...
  /**
   * 文档小节「layout=center」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·layout=center", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "layout=center";
  ...
  /**
   * 文档小节「layout=left（文案左对齐 + 背景图）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·layout=left（文案左对齐）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "layout=left（文案左对齐 + 背景图）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「layout=right（文案右对齐 + 背景图）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·layout=right（文案右对齐）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "layout=right（文案右对齐 + 背景图）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
