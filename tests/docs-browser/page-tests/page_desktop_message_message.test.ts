/**
 * @fileoverview 文档路由 `/desktop/message/message` 的浏览器测试。
 * Message 全局提示 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/message/message";

describe("文档页 E2E：/desktop/message/message（Message 全局提示）", () => {
  ...
  /**
   * 点「成功」触发 message，页面应出现「保存成功」。
   */
  it("点击成功触发 Message 文案", async (t) => {
    if (!t?.browser?.goto) return;
    ...
    await env.delay(250);
    const hasText = await t.browser.evaluate(() => {
  ...
  /**
   * 文档小节「placement 显示位置」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·placement 显示位置", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "placement 显示位置";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「duration 自定义时长」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·duration 自定义时长", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "duration 自定义时长";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「destroy 关闭全部」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·destroy 关闭全部", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "destroy 关闭全部";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
