/**
 * @fileoverview 文档路由 `/desktop/message/toast` 的浏览器测试。
 * Toast 轻提示 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/message/toast";

describe("文档页 E2E：/desktop/message/toast（Toast 轻提示）", () => {
  ...
  /**
   * 点示例区「success」按钮，页面应出现「操作成功」文案（Toast）。
   */
  it("点击 success 触发 Toast 文案", async (t) => {
    if (!t?.browser?.goto) return;
    ...
    const hasText = await t.browser.evaluate(() => {
      return document.body.innerText.includes("操作成功");
    }) as boolean;
  ...
  /**
   * 文档小节「placement 与 duration」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·placement 与 duration", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "placement 与 duration";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
