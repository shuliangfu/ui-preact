/**
 * @fileoverview 文档路由 `/desktop/navigation/breadcrumb` 的浏览器测试。
 * Breadcrumb 面包屑 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/navigation/breadcrumb";

describe("文档页 E2E：/desktop/navigation/breadcrumb（Breadcrumb 面包屑）", () => {
  ...
  /**
   * 文档小节「自定义 separator」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·自定义 separator", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "自定义 separator";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「onItemClick（无 href 项点击）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·onItemClick（无 href 项点击）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "onItemClick（无 href 项点击）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
