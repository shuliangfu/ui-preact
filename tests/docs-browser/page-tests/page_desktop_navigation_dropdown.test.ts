/**
 * @fileoverview 文档路由 `/desktop/navigation/dropdown` 的浏览器测试。
 * Dropdown 下拉 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/navigation/dropdown";

describe("文档页 E2E：/desktop/navigation/dropdown（Dropdown 下拉）", () => {
  ...
  /**
   * 文档小节「trigger=click」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·trigger=click", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "trigger=click";
  ...
  /**
   * 文档小节「trigger=hover」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·trigger=hover", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "trigger=hover";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「overlay 复杂示例（头像、分割线、图标菜单项）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·overlay 复杂示例（头像、分割线、图标菜单项）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "overlay 复杂示例（头像、分割线、图标菜单项）";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
