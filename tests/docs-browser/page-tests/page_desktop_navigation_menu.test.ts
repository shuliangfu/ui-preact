/**
 * @fileoverview 文档路由 `/desktop/navigation/menu` 的浏览器测试。
 * Menu 菜单 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
describe("文档页 E2E：/desktop/navigation/menu（Menu 菜单）", () => {
  ...
  /**
   * 文档小节「mode=vertical（垂直）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  ...
  /**
   * 文档小节「mode=horizontal + usePopoverSubmenu」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  /**
   * 回归：垂直 defaultOpenKeys 展开后须能收起（曾因外层 effect 重置内部 Signal 无法收起）。
   */
  it("垂直示例：点击子菜单标题应收起（aria-expanded=false）", async (t) => {
    ...
    await t.browser.evaluate(() => {
      ...
      for (let i = 0; i < heads.length; i++) {
        ...
        if (!nav) return;
        const triggers = nav.querySelectorAll(
          'button[type="button"][aria-haspopup="true"]',
        );
        for (let j = 0; j < triggers.length; j++) {
          const b = triggers[j] as HTMLButtonElement;
  ...
  /**
   * 回归：水平 popover 子菜单须能展开（曾因 popoverOpenKeysRef 被整组件重跑重置无法展开）。
   */
  it("水平 popover 示例：点击子菜单后须出现子项文案", async (t) => {
    ...
    const hasSubItem = await t.browser.evaluate(() => {
      ...
      for (let i = 0; i < heads.length; i++) {
        ...
        return (section.innerText || "").includes("子项 3");
      }
      return false;
    });
