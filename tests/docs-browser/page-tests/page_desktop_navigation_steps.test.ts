/**
 * @fileoverview 文档路由 `/desktop/navigation/steps` 的浏览器测试。
 * Steps 步骤条 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/navigation/steps";

describe("文档页 E2E：/desktop/navigation/steps（Steps 步骤条）", () => {
  ...
  /**
   * 文档小节「direction=horizontal（可点击）」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·direction=horizontal（可点击）", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "direction=horizontal（可点击）";
  ...
  /**
   * 文档小节「direction=vertical」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·direction=vertical", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "direction=vertical";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
  ...
  /**
   * 文档小节「items 含 status」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·items 含 status", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
    await env.delay(520);
    const ok = await t.browser.evaluate(() => {
      const needle = "items 含 status";
      const main = document.querySelector("main");
      if (!main) return false;
      const heads = main.querySelectorAll("h2, h3");
      for (let i = 0; i < heads.length; i++) {
