/**
 * @fileoverview 文档路由 `/desktop/other/back-top` 的浏览器测试。
 * BackTop 回到顶部 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/other/back-top";

describe("文档页 E2E：/desktop/other/back-top（BackTop 回到顶部）", () => {
  ...
  /**
   * 文档小节「默认用法」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·默认用法", async (t) => {
    if (!t?.browser?.goto) return;
  ...
  /**
   * 文档小节「自定义位置」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·自定义位置", async (t) => {
    if (!t?.browser?.goto) return;
    await env.goto(t, DOC_PATH);
