/**
 * @fileoverview 文档路由 `/desktop/user/e2e-1` 的浏览器测试。
 * 动态路由用户页 — **本文件仅服务该路由**：后续请按文档示例块继续补全分步交互与断言，勿用仓库脚本覆盖正文。
...
const DOC_PATH = "/desktop/user/e2e-1";

describe("文档页 E2E：/desktop/user/e2e-1（动态路由用户页）", () => {
  ...
  /**
   * 文档小节「用户详情」：在 main 内定位对应 h3，于该示例区块内完成一次交互。
   */
  it("严格·用户详情", async (t) => {
    if (!t?.browser?.goto) return;
